...
onMounted(() => {
    focal_role.value = props.auth.user.focal_role;
});
...
function filtersearchInput(){
    router.get(route('focal.transitioning'),
...
const endorseForm = reactive({
    beneficiaries: [],
    from_benefeciary_type: props.auth.user.focal_role,
    barangay_id: props.auth.user.barangay_id
});
...
watch(() => endorseForm.beneficiaries.length, (newLength) => {
    if (newLength != props.benefeciaries.length) {
...
const endorseBeneficiary = () => {
    router.post(route("focal.endorse_transitioning"), endorseForm, {
        onSuccess: (page) => {
            resetEndorseForm(),
                Swal.fire({
                    toast: true,
...
const retryEndorseBeneficiary = (id) => {
    router.put(route("focal.retry_endorse_transitioning", id));
};

const cancelEndorseBeneficiary = (id) => {
    router.delete(route("focal.cancel_endorse_transitioning", id));
};
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr v-for="benefeciary in benefeciaries" :key="benefeciary.id"
                                    ...
                                    <td class="px-4 py-3">
                                        <input type="checkbox" :value="benefeciary.id"
                                            v-model="endorseForm.beneficiaries">
                                    </td>
                                </tr>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr v-for="benefeciary in endorsed_benefeciaries" :key="benefeciary.id"
                                    ...
                                    <td class="px-4 py-3">
                                        <span v-if="benefeciary.status == 0" class="badge badge-warning">Pending</span>
                                        <span v-if="benefeciary.status == 1" class="badge badge-warning">Forwarded</span>
                                        <span v-if="benefeciary.status == 3" class="badge badge-error">Rejected</span>
                                    </td>
                                    <td class="px-4 py-3 flex gap-2 items-center justify-start">
                                        <button v-if="benefeciary.status == 3" type="button" class="text-blue-600" @click="retryEndorseBeneficiary(benefeciary.id)"><svg
                                        ...
                                        <button v-if="benefeciary.status < 1" type="button" class="text-red-600" @click="cancelEndorseBeneficiary(benefeciary.id)"><svg
                                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
