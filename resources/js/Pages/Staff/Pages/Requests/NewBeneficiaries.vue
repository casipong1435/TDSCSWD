...
function filtersearchInputRequest() {
    router.get(route('staff.new_beneficiaries'),
...
const respondRequest = () => {
    try {
        router.put(
            route("staff.respondRequest"), response, {
            onProgress: () => {
...
function viewStatusInfo(request) {
    console.log(request);
    isStatusInfoModal.value = true;
    getRequestInfo(request);
}

function cancelViewStatusInfo() {
                ...
                <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr v-for="request in made_requests" :key="request.id"
                                    ...
                                    <td class="px-4 py-3">{{ getBenefeciaryType(request.benefeciary.benefeciary_type) }}
                                    </td>
                                    <td class="px-4 py-3">{{ request.user.name }}</td>
                                    <td class="px-4 py-3">
                                        <span class="badge cursor-pointer" @click="viewStatusInfo(request)" :class="{
                                    ...
                                    <td class="px-4 py-3 gap-2 flex flex-row">
                                        <button type="btn" class="text-blue-700" @click="
                                            openEditModal(request.benefeciary)
                                        ...
                                        <button type="btn" class="text-green-700"
                                            @click="openResponseModal(request.id, 2)" v-if="request.status == 1">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <button v-if="request.status == 1" type="btn" class="text-red-700"
                                            @click="openResponseModal(request.id, 4)"><svg
                                        ...
                                        <button v-if="request.status == 2"
                                            type="btn" class="text-red-700"
                                            @click="openResponseModal(request.id, 1)"><svg
                                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <button v-if="request.status == 4 && request.rejected_by == 'Staff'"
                                            type="btn" class="text-red-700"
                                            @click="openResponseModal(request.id, 5)"><svg
                                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M6 18 18 6M6 6l12 12" />
                                ...
                                <tr v-if="made_requests.length <= 0">
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 v-if="response.status == 1"
                            class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                            Are you sure you want to cancel the verification of this endorsement?
                        </h3>

                        <h3 v-else-if="response.status == 5"
                            class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                            Are you sure you want to cancel the rejection of this endorsement?
                        </h3>
                        ...
                        <ProcessingButton type="button" :process="processing" :text="'Confirm'"
                            ...
                            :class="{ 'bg-red-600 hover:bg-red-800': response.status == 4, 'bg-green-600 hover:bg-green-800': response.status == 2, 'hover:bg-blue-800 bg-blue-600': response.status == 1 }" />
                        <button type="button" @click="closeResponseModal()"
