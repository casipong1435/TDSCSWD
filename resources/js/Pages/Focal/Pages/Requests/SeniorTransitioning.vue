...
const getBenefeciaryData = (benefeciary) => {
    ...
    switch (form.benefeciary_type) {
        ...
        //PWD
        case 2:
            form.pwd_id_number = benefeciary?.benefeciary?.pwd?.pwd_id_number?? null;
            form.date_pic_issued = benefeciary?.benefeciary?.pwd?.date_pic_issued?? null;
            form.pic_status = benefeciary?.benefeciary?.pwd?.pic_status?? null;
            break;
        //Women
        case 3:
            form.kalipi_id_number = benefeciary?.benefeciary?.women?.kalipi_id_number?? null;
            form.date_kic_issued = benefeciary?.benefeciary?.women?.date_kic_issued?? null;
            form.kic_status = benefeciary?.benefeciary?.women?.kic_status?? null;
            break;
    }
...
const respondRequest = () => {
    ...
    router.put(route('focal.put-respond-transition', { id:requestID.value, status: responseStatus.value }), {}, {
        onSuccess: page => {
            Swal.fire({
                toast: true,
                icon: 'success',
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr v-for="benefeciary in endorsed_benefeciaries" :key="benefeciary.id"
                                    ...
                                    <td class="px-4 py-3">
                                        <span class="badge"
                                            :class="{ 'badge-warning': benefeciary.status < 2, 'badge-error': benefeciary.status == 3}">
                                            {{ benefeciary.status == 0 ? 'Pending' : benefeciary.status == 1 ? 'Forwarded' : 'Rejected' }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 flex gap-2 items-center justify-start" >
                                        <button type="button" class="text-blue-600" v-if="benefeciary.status == 0"
                                        ...
                                        <button type="button" class="text-green-600" v-if="benefeciary.status == 0" @click="openAcceptRequestModal(benefeciary.id)"><svg
                                        ...
                                        <button type="button" class="text-red-600" v-if="benefeciary.status == 0" @click="openRejectRequestModal(benefeciary.id)"><svg
                                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                            Are you sure you want to {{ responseStatus == 1 ? 'accept' :  responseStatus == 3 ? 'reject' : 'retry endorsing' }} this request?
                        </h3>
                        <ProcessingButton type="button" :process="processing" :text="'Confirm'" class="btn"
                            :class="{ 'bg-green-600 hover:bg-green-800': responseStatus == 1, 'bg-red-600 hover:bg-red-800': responseStatus == 3, 'bg-blue-600 hover:bg-blue-800': responseStatus == 0 }"
