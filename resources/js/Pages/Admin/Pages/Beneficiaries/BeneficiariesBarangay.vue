...
function filter() {
    router.get(route('admin.beneficiary_barangay', usePage().props.barangay.id),
        {
...
const getBenefeciaryData = (benefeciary) => {
    ...
    birthday_data.value = benefeciary.birthday_data;
    program.value = benefeciary.program_beneficiary;
            ...
            <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
                ...
                <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden p-3">
                    ...
                    <div class="flex md:justify-between md:items-center mb-4 flex-col md:flex-row">
                        <div class="text-2xl mb-4 sm:mb-0">Beneficiaries List <span class="text-sm">(Barangay {{
                            barangay.barangay_name }})</span></div>
                        <div class="gap-2 grid grid-cols-1 sm:grid-cols-2">

                            <div class="relative w-full  col-span-2 md:col-span-1 flex justify-end">
                                <button type="button" @click="filter()"
                                    class="btn w-full md:max-w-50 btn-primary">Search</button>
                            </div>

                            <div class="relative w-full col-span-2 md:col-span-1">
                                <!-- Trigger Button -->
                                <button @click="toggleCategoryDropdown"
                                    class=" w-full md:max-w-sm px-4 py-2 text-left border border-gray-300 rounded-md shadow-sm bg-white focus:outline-none">
                ...
                <div class="relative p-4 w-full max-w-2xl max-h-full">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="p-4 md:p-5 space-y-4">
                            ...
                            <div class="mt-2">
                                <div class="text-xl mb-2">Benefits Received</div>
                                <div class="w-full px bg-gray-50">
                                    <div v-if="program.length <= 0 && birthday_data.length <= 0" class="text-center">No
                                        Benefits Received</div>
                                    <div class="text-start">
                                        <div v-if="birthday_data.length > 0" class="px-2">
                                            <div class="text-lg mb-2 font-bold">
                                                Birthday Gifts
                                            </div>
                                            <ul>
                                                <li class="pl-3" v-for="gift in birthday_data" :key="gift.id">Received a gift from CSWD on {{ new Date(gift.date_approved).toDateString() }}</li>
                                            </ul>
                                        </div>
                                        <div v-if="program.length > 0" class="px-2">
                                            <div class="text-lg mb-2 font-bold">
                                                In  Programs
                                            </div>
                                            <ul>
                                                <li class="pl-3" v-for="program_data in program" :key="program_data.program.id">Beneficiary of  {{ program_data.program.title + ' on ' + new Date(program_data.program.start_date).toDateString() }}</li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
