...
function filtersearchInput() {
    router.get(route('staff.request_info', usePage().props.request_info.id),
        {
...
const requestID = usePage().props.requestID;

const respondRequest = () => {
    processing.value = true;
    router.put(route('staff.respondRequest', { id: requestID, status: responseStatus.value }), {}, {
        onSuccess: page => {
            Swal.fire({
                toast: true,
                icon: 'success',
...
function openAcceptRequestModal() {
            ...
            <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
                <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    <div class="flex justify-between items-center gap-3 px-2">
                        <div class="text-2xl p-2 mb-2">{{ request_info.name }}</div>
                        <button type="button" class="flex flex-row" @click="viewStatusInfo()">
                            <span class="mr-2" :class="{
                                'text-orange-500': request_info.status < 2,

                                'text-red-500': request_info.status == 4, 'text-green-500': request_info.status == 3 || request_info.status == 2
                            }">{{
                                    getRequestStatusName(request_info.status) }}</span>
                            <span :class="{
                                'text-orange-500': request_info.status < 2,
                                'text-red-500': request_info.status == 4, 'text-green-500': request_info.status == 3 || request_info.status == 2
                    ...
                    <div class="breadcrumbs text-sm">
                        <ul>
                            <li>
                                <Link :href="route('staff.new_beneficiaries',)">Requests</Link>
                            </li>
                            <li>
                                <a>{{ request_info.name }}</a>
                            </li>
                        </ul>
                    </div>
                    <div
                        ...
                        <div
                            ...
                            <button v-if="request_info.status < 2" type="button" @click="openAcceptRequestModal"
                                class="flex items-center justify-center text-white bg-green-800 hover:bg-green-900 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                            ...
                            <button v-if="request_info.status < 2" type="button" @click="openRejectRequestModal"
                                class="flex items-center justify-center text-white bg-red-800 hover:bg-red-900 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="size-5">
                            ...
                            <button v-if="request_info.status == 2 || request_info.status == 4" type="button"
                            ...
                            <tbody>
                                <tr v-for="benefeciary in benefeciaries" :key="benefeciary.id"
                                    ...
                                    <td class="px-4 py-3 flex flex-row gap-3">
                                        <button type="button" @click="openEditModal(benefeciary)"
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                            Are you sure you want to {{ responseStatus == 1 ? 'accept' : responseStatus == 4 ? 'reject'
                                : 'cancel' }} this request?
                        </h3>
                        <ProcessingButton type="button" :process="processing" :text="'Confirm'" class="btn"
                            :class="{ 'bg-green-600 hover:bg-green-800': responseStatus == 1, 'bg-red-600 hover:bg-red-800': responseStatus == 4 || responseStatus == 0 }"
