...
function filtersearchInputRequest() {
    router.get(route('admin.request.beneficiaries'),
...
const respondRequest = () => {
    try {
        router.put(
            route("admin.respondRequest"), response, {
            onProgress: () => {
                                ...
                                <tr v-for="request in made_requests" :key="request.id"
                                    ...
                                    <td class="px-4 py-3 gap-2 flex flex-row">
                                        ...
                                        <button type="btn" class="text-green-700"
                                            @click="openResponseModal(request.id, 3)" v-if="request.status == 2">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <!--Reject Button-->
                                        <button v-if="request.status == 2" type="btn" class="text-red-700"
                                        ...
                                        <!--Cancel Reject-->
                                        <button v-if="request.status == 4 && request.rejected_by == 'Admin'" type="btn"
                                            class="text-red-700" @click="openResponseModal(request.id, 2)"><svg
                                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 v-else-if="response.status == 2"
                            class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                            Are you sure you want to cancel the rejection of this
                            endorsement?
                        </h3>
                        ...
                        <ProcessingButton type="button" :process="processing" :text="'Confirm'"
                            ...
                            :class="{ 'bg-red-600 hover:bg-red-800': response.status == 4, 'bg-green-600 hover:bg-green-800': response.status == 3, 'hover:bg-blue-800 bg-blue-600': response.status == 2 }" />
                        <button type="button" @click="closeResponseModal()"
