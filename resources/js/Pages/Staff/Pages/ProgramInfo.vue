...
const selectedBeneficiaryItems = ref([]);
...
const isAllBeneficiarySelected = ref(false);

const handleSelectAllBeneficiary = () => {
    if (isAllBeneficiarySelected.value) {
        selectedBeneficiaryItems.value = [];
    } else {
        selectedBeneficiaryItems.value = usePage().props.beneficiaries.map((beneficiary) => beneficiary.id);
        console.log(selectedBeneficiaryItems.value);
    }
    isAllBeneficiarySelected.value = !isAllBeneficiarySelected.value;

};

function filter() {
    router.get(route('staff.program_info', usePage().props.program.id),
        {
...
const addProgramBeneficiary = () => {
    processing.value = true;
    const formData = new FormData();
    formData.append('beneficiaries', selectedBeneficiaryItems.value);
    formData.append('program_id', usePage().props.program.id);
    router.post(route("staff.add-program-beneficiary"), formData, {
        onSuccess: (page) => {
            Swal.fire({
                toast: true,
                icon: "success",
...
watch(() => selectedBeneficiaryItems.value.length, (newLength) => {
    if (newLength > 0) {
        canAdd.value = true;
    } else {
        canAdd.value = false;
...
const submitList = () => {
    processing.value = true;
    router.put(route("staff.submit-program-beneficiary", usePage().props.program.id), {}, {
        onSuccess: (page) => {
            processing.value = false;
            Swal.fire({
                toast: true,
...
const removeProgramBeneficiary = (id) => {
    router.delete(route('staff.remove-program-beneficiary', id));
};

const cancelListSubmission = (id) => {
    router.put(route('staff.cancel-list-submission', id));
};
        ...
        <section class="bg-gray-50 dark:bg-gray-900 p-4 sm:p-5">
            <div class="mx-auto max-w-screen-xl px-4 lg:px-12 mb-10">
                <div class="breadcrumbs text-sm mb-2">
                    <ul>
                        <li>
                            <Link :href="route('staff.programs')">Program</Link>
                        </li>
                        <li>
                            <a>Program Info</a>
                        </li>
                    ...
                    <div class="col-span-2 md:col-span-1 py-2">
                        <div class="shadow-lg p-3 rounded gap-y-3">
                            <div class="flex justify-between items-center py-2 border-b">
                                ...
                                <a target="_blank" :href="route('staff.individualProgramReport', usePage().props.program.id)" class="text-red-700 hover:text-red-900">
                            ...
                            <div class="flex flex-row justify-between py-2 gap-5">
                                ...
                                <div> {{ program_beneficiaries.length }}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="mx-auto max-w-screen-xl px-4 lg:px-12" v-if="program.status >= 1">
                <div class="tabs mb-2">
                    <div class="tab tab-bordered px-6" :class="{ 'tab-active': inSelection }"
                        @click="inSelection = true">
                        List of Beneficiaries
                    </div>
                    <div class="tab tab-bordered px-6" :class="{ 'tab-active': !inSelection }"
                        @click="inSelection = false">
                        Selected Beneficiaries
                    </div>
                </div>
                <div v-if="inSelection"
                    class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden p-3">
                    <div class="flex md:justify-between md:items-center mb-4 flex-col md:flex-row"
                        v-if="program.status == 1">
                        <div class="text-2xl mb-4 sm:mb-0">Beneficiaries List</div>
                        <div class="gap-2 grid grid-cols-2 sm:grid-cols-3">
                            <div class="relative w-full  col-span-2 sm:col-span-1 flex justify-end">
                                <button type="button" @click="filter()"
                                    class="btn w-full sm:w-50 btn-primary">Search</button>
                            </div>
                            <div class="relative w-full max-w-sm col-span-2 sm:col-span-1">
                                <!-- Trigger Button -->
                                ...
                                <div v-if="dropdownBarangayOpen"
                                    ...
                                    <div class="p-2 gap-y-2 max-h-40 overflow-y-auto">
                                        ...
                                        <label v-for="(barangay, index) in barangays" :key="index"
                            ...
                            <div class=" p-3 rounded gap-y-3">
                                <div class="overflow-x-auto">
                                    <div
                                        ...
                                        <ProcessingButton :process="processing" :text="'+ Add'" :disabled="!canAdd"
                                            @click="addProgramBeneficiary" />
                                    ...
                                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                        <thead
                                            ...
                                            <tr>
                                                <th scope="col" class="px-4 py-3">
                                                    <span>
                                                        <input type="checkbox" class="mr-3" id="allBeneficiary"
                                                            :check="isAllBeneficiarySelected"
                                                            @change="handleSelectAllBeneficiary">
                                                        <label for="allBeneficiary">All</label>
                                                    </span>
                                                </th>
                                        ...
                                        <tbody>
                                            <tr class="border-b dark:border-gray-700"
                                                ...
                                                <td class="px-4 py-3"><input type="checkbox" :value="beneficiary.id"
                                                        v-model="selectedBeneficiaryItems"></td>
                                                <th scope="row"
                    ...
                    <div v-else-if="program.status == 0 || program.status >= 2"
                        class="p-5 w-full h-full flex justify-center items-center">
                        <div class="text-2xl">Cannot Select Beneficiary!</div>
                    </div>
                ...
                <div v-else class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    ...
                    <div
                        ...
                        <div v-if="program.status == 1"
                            class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                            <ProcessingButton :process="processing" :text="'Submit List'"
                                :class="{ 'disabled opacity-80 pointer-events-none': program_beneficiaries.length <= 0 }"
                                @click="submitList()" />

                        </div>

                        <div v-if="program.status == 2"
                            class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                            <ProcessingButton :process="processing" class="bg-red-600 hover:bg-red-800" :text="'Cancel'"
                                @click="cancelListSubmission(program.id)" />
                    ...
                    <div class="overflow-x-auto">
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr v-for="benefeciary in program_beneficiaries" :key="benefeciary.id"
                                    class="border-b dark:border-gray-700">
                                    <th scope="row"
                                        class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{
                                            getFullName(benefeciary.beneficiary.first_name,
                                                benefeciary.beneficiary.middle_name,
                                                benefeciary.beneficiary.last_name)
                                        }}</th>
                                    <td class="px-4 py-3">{{ benefeciary.beneficiary.age }}</td>
                                    <td class="px-4 py-3">{{ benefeciary.beneficiary.sex }}</td>
                                    <td class="px-4 py-3">{{ benefeciary.beneficiary.date_of_birth }}</td>
                                    <td class="px-4 py-3">{{ benefeciary.beneficiary.street }}</td>
                                    <td class="px-4 py-3 gap-2 flex flex-row">
                                        <button type="btn" class="text-blue-700"
                                            @click="openEditModal(benefeciary.beneficiary)"><svg
                                        ...
                                        <button v-if="program.status < 2" type="btn" class="text-red-700"
                                            @click="removeProgramBeneficiary(benefeciary.id)"><svg
                                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <tr v-if="program_beneficiaries.length <= 0">
