...
function filtersearchInputRequest() {
    router.get(route('barangay.new_beneficiary'),
...
const respondRequest = () => {
    try {
        router.put(
            route("barangay.respondRequest"), response, {
            onProgress: () => {
                                ...
                                <tr v-for="request in made_requests" :key="request.id"
                                    ...
                                    <td class="px-4 py-3 gap-2 flex flex-row">
                                        ...
                                        <button type="btn" class="text-green-700"
                                            @click="openResponseModal(request.id, 1)" v-if="request.status == 0">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <button v-if="request.status == 0" type="btn" class="text-red-700"
                                        ...
                                        <button
                                            v-if="(request.status == 4 && request.rejected_by == 'Barangay') || request.status == 1"
                                            type="btn" class="text-red-700"
                                            @click="openResponseModal(request.id, 0)"><svg
                                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-6">
                                                <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <h3 v-if="response.status == 0"
                            class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                            Are you sure you want to undo the action of this endorsement?
                        </h3>
                        ...
                        <ProcessingButton type="button" :process="processing" :text="'Confirm'"
                            ...
                            :class="{ 'bg-red-600 hover:bg-red-800': response.status == 4, 'bg-green-600 hover:bg-green-800': response.status == 1, 'hover:bg-blue-800 bg-blue-600': response.status == 0 }" />
                        <button type="button" @click="closeResponseModal()"
